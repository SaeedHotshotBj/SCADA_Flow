...
<div class="machineTrendWindow">
  ...
  <div class="machineTrendChart"><canvas id="machineTrendCanvas"></canvas></div>
    ...
    async function loadTrend() {
        ...
        status.textContent = "در حال خواندن داده از PLC_Data...";
        ...
        try {
            const response = await fetch("/machine_trend_data?_=" + Date.now(), {
                ...
                body: JSON.stringify({
                    tag: selected.tag,
                    start: document.getElementById("machineTrendStart").value.trim(),
                    end: document.getElementById("machineTrendEnd").value.trim()
                })
            });
    ...
    function draw(data) {
        ...
        status.textContent = "تعداد نقاط: " + points.length + " | CompanyID: " + data.CompanyID;
    }
