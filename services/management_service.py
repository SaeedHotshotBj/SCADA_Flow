...
def _management_configs(company_id):
    ...
    for node in nodes.values():
        if not isinstance(node, dict) or str(node.get("name", "")).strip() != "ManagementPanel":
            continue
...
def _management_config(company_id):
    """Return one effective ManagementPanel config without losing earlier
    settings when another ManagementPanel node is empty or partial."""
            ...
            if key == "calculations":
                ...
                for item in value:
                    ...
                    if not name or not expression:
                        continue
                    key_name = name.lower()
                    if key_name not in calculation_map:
                        calculation_order.append(key_name)
                    calculation_map[key_name] = dict(item)
                continue

            # Do not let empty/blank later nodes erase a valid setting.
            if value not in (None, "", [], {}):
                merged[key] = value

    if calculation_order:
        merged["calculations"] = [calculation_map[name] for name in calculation_order]
    elif "calculations" not in merged:
        merged["calculations"] = []
...
def ensure_management_tables():
    ...
    try:
        ...
        CREATE INDEX IF NOT EXISTS idx_contracts_company_date ON Contracts (CompanyID, ContractDate);
        CREATE INDEX IF NOT EXISTS idx_contract_products_contract ON ContractProducts (ContractID, ProductID);
        CREATE INDEX IF NOT EXISTS idx_products_company_code ON Products (CompanyID, ProductCode);
        CREATE INDEX IF NOT EXISTS idx_product_bom_product ON ProductBOM (ProductID);
        """)
        conn.commit()
        TABLES_READY = True
    finally:
...
def save_product(company_id, payload):
    ...
    try:
        ...
        if row is None:
            cursor = conn.execute(
                "INSERT INTO Products (CompanyID, ProductCode, ProductName, Unit) VALUES (?, ?, ?, ?)",
            ...
            conn.execute(
                "UPDATE Products SET ProductName=?, Unit=CASE WHEN ?<>'' THEN ? ELSE Unit END WHERE ProductID=?",
        ...
        if bom is None:
            conn.execute(
                "INSERT INTO ProductBOM (ProductID, CostPerKg, CostPerMeter, Notes, UpdatedAt) VALUES (?, ?, ?, ?, datetime('now','localtime'))",
                (product_id, kg, meter, notes),
            )
        else:
            conn.execute(
                "UPDATE ProductBOM SET CostPerKg=?, CostPerMeter=?, Notes=?, UpdatedAt=datetime('now','localtime') WHERE BOMID=?",
...
def save_contract(company_id, payload):
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO Contracts (CompanyID, ContractCode, ContractDate, ContractName, DeliveryDate, Description) VALUES (?, ?, ?, ?, NULL, ?)",
        ...
        for item in products:
            ...
            if product is None:
                ...
                product = conn.execute("SELECT ProductID FROM Products WHERE ProductID=?", (cursor.lastrowid,)).fetchone()
            else:
                conn.execute(
                    "UPDATE Products SET ProductName=?, Unit=CASE WHEN ?<>'' THEN ? ELSE Unit END WHERE ProductID=?",
            ...
            conn.execute(
                "INSERT INTO ContractProducts (ContractID, ProductID, OrderedQuantity, DeliveryDate, Description) VALUES (?, ?, ?, ?, ?)",
                (contract_id, int(product["ProductID"]), ordered, delivery, str(item.get("description", "")).strip()),
            )
            inserted += 1
        if inserted == 0:
...
def _management_calculations(company_id):
    configs = _management_configs(company_id)
    merged = _management_config(company_id)
    rows = merged.get("calculations", []) if isinstance(merged, dict) else []
    result = []
    seen = set()
    for item in rows:
        if not isinstance(item, dict):
