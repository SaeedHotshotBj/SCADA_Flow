...
def get_report_products(company_id):
    """ONLY ReportOutput.products define report columns."""
    products = []
    try:
...
def save_report_snapshot(company_id, tags, report_products, timestamp=None):
    ...
    for product in report_products:
        ...
        if not requested or requested.lower() in seen:
            continue
        seen.add(requested.lower())
        item = tag_lookup.get(requested.lower())
        ...
        try:
            ...
            continue
        values.append((item[0], numeric_value))
    ...
    try:
        cursor.execute(
            "INSERT INTO ReportHistory (CompanyID, Timestamp) VALUES (?, ?)",
...
def get_report_data(company_id, start, end):
    """Return only ReportOutput-configured columns and their data."""
    ...
    try:
        cursor.execute(
            f"""
            SELECT h.ReportID, h.Timestamp, v.TagName, v.Value
    ...
    for row in fetched:
        ...
        item = grouped.setdefault(
            report_id,
            {"timestamp": str(row["Timestamp"]), "values": [None for _ in products]},
        )
