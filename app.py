...
def _auth_no_cache(response):
    ...
    if request.path in protected_paths or request.path.startswith("/dashboard/"):
...
def _get_roles_engaged_for_page(company_id, page_node_name):
    ...
    target_node_ids = [str(node_id) for node_id, node in nodes.items() if node.get("name") == page_node_name]
    ...
    selected_roles = []
    for engaged_node in engaged_nodes:
        ...
        if not isinstance(roles, list):
            continue
        for role in roles:
            role_name = str(role.get("role", "") if isinstance(role, dict) else role).strip()
            if role_name:
    ...
    for role in selected_roles:
        if role not in result:
            result.append(role)
    print("FLOW ACCESS:", page_node_name, "Company:", company_id, "Allowed Roles:", result)
    return result
...
def dashboard_config():
    ...
    if company_id is None:
        return jsonify([]), 403
    try:
        return jsonify(get_dashboard_widgets(company_id))
    except Exception as e:
        import traceback
        traceback.print_exc()
        return jsonify({"error": "Dashboard configuration error", "details": str(e)}), 500
...
def receive_edge_data():
    try:
        ...
        # Existing implementation continues below in the deployed repository.
        return jsonify({"status": "ok"})
    except Exception as e:
        print("EDGE DATA ERROR:", e)
